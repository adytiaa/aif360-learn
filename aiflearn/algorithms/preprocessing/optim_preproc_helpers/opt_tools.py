...
class OptTools():
    ...
    def get_mask(self, dfRef):
        ...
        dfRows = (pd.DataFrame(index=dfRef.index)
                  .reset_index()[target_ix].values)
        dfCols = (pd.DataFrame(index=dfRef.columns)
                  .reset_index()[target_ix].values)
    ...
    def set_distortion(self, get_distortion, clist=[]):
        ...
        rows_dict = [{self.XY_features[i]:t[i]
                      for i in range(len(self.XY_features))}
                     for t in rows_tuple]
    ...
    def optimize(self, epsilon=1., dlist=[], verbose=True):
        r"""Main optimization routine to estimate the pre-processing
        transformation.
        ...
        # print(PxyMarginal)
        # Pxy_xhyh = np.nan_to_num(np.diag(PxyMarginal**(-1)))
        # .dot(self.dfMask_Pxyd_to_Pxy.values.T)
        # .dot(np.diag(PxydMarginal))*Pmap
    ...
    def compute_marginals(self):
        ...
        self.dfPxygdPrior = self.dfPxygdPrior.div(
            self.dfPxygdPrior.sum(axis=1), axis=0)
